...
gen_input(int element_count, uint64_t seed) //gen_input is a function that generates random key-value pairs with given element_count and seed
{
...
{
	assert(argc == 4); //assert is a function that checks if the condition is true. If it is not true, the program will terminate with a message.
	int element_count = std::atoi(argv[1]); //atoi is a function that converts a string to an integer
	auto seed = std::atoll(argv[2]); //atoll is a function that converts a string to a long long
